...
- Make it optional to build taflzero.
...
- The brandubh rules.
- Don't pass the board size in play (breaks Brandubh).
...
- hnefatafl-client: you have to have a username to press "Login" or "Create
  Account".
- hnefatafl-client: don't delete the username when you fail to login.
...
- The initial display (the old client won't show anything for initial
  display).
...
- hnefatafl-client: mark the winner in archived games.
- End the game when the defender can not escape.
...
- hnefatafl-client: decrease the padding on tab labels.
...
- hnefatafl-client: put users only on the Users screen.
...
- hnefatafl-server-full: actually delete unused accounts.
...
- hnefatafl-server-full: Set the turn correctly in games light when restarting the server.
...
- hnefatafl-client: make the spacing and padding slightly smaller.
- hnefatafl-client: always put resign and request draw on the same line.

### Fixed

- Create a data local directory not a data directory.
...
[unreleased]: https://codeberg.org/dcampbell/hnefatafl/compare/v6.4.1...main
[6.4.1]: https://codeberg.org/dcampbell/hnefatafl/compare/v6.4.0...v6.4.1
[6.4.0]: https://codeberg.org/dcampbell/hnefatafl/compare/v6.3.0...v6.4.0
