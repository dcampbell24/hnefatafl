        ...
        for (x, letter) in letters.iter().enumerate() {
            ...
            for y in 0..board_size_usize {
                ...
                let mut txt = match board.get(&vertex) {
                    Space::Attacker => text(&self.chars.attacker).font(CHESS_FONT),
                    Space::Defender => text(&self.chars.defender).font(CHESS_FONT),
                    Space::Empty => {
                    ...
                    Space::King => text(&self.chars.king).font(CHESS_FONT),
                };
