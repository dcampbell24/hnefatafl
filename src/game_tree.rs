...
impl Tree {
    ...
    #[must_use]
    pub fn basic_tree_search(&mut self, _duration: Duration, _depth: u8) -> (u64, Vec<Node>) {
        // let t0 = Instant::now();
        let mut loops = 0;
        // Fixme:
        ...
        for _ in 0..1 {
            let children = if self.arena[&self.here].children.is_empty() {
                vec![self.here]
            } else {
                self.arena[&self.here].children.clone()
            };

            for here in &children {
                /* Fixme:
                let t1 = Instant::now();
